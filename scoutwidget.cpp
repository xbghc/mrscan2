...
QList<QPointF> getLineInViewport(double a, double b, double c, double viewFov,
                                 double lineFov) {
    // ax + by + c = 0
...
void ScoutWidget::previewSlice(double fov, QVector3D angles,
     ...
     * First calculate the normal vector v of the slice based on angles,
     * v[0]*x+v[1]*y+v[2]*z+c=0 Scout has an offset relative to the origin,
     * viewed as center in graphicScene, equivalent to slice offset in opposite
     * direction, so slice offset is offsets - m_offsets[i] Scout is parallel to
     * a coordinate axis, so one of x,y,z equals 0, thus we have ax + by + c =
     * 0, where x and y have different meanings than in 3D The x and y discussed
     * below are in the scout plane rather than in 3D Scout value range is
     * (-m_fov/2, m_fov/2)
     */
        auto v = getVector(angles);
        auto c = (m_offsets[i][0] - offsets[0]) * v[0] +
                 (m_offsets[i][1] - offsets[1]) * v[1] +
                 (m_offsets[i][2] - offsets[2]) * v[2];
        ...
        if (axis[0] != 0) {
            points =
                getLineInViewport(v[1] * axis[0], v[2] * axis[0], c, m_scoutFov, fov);
        } else if (axis[1] != 0) {
            points =
                getLineInViewport(v[0] * axis[1], v[2] * axis[1], c, m_scoutFov, fov);
        } else if (axis[2] != 0) {
            points =
                getLineInViewport(v[0] * axis[2], v[1] * axis[2], c, m_scoutFov, fov);
        }

        auto line = new QGraphicsLineItem(points[0].x(), points[0].y(),
