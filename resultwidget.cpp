...
ResultWidget::ResultWidget(QWidget *parent)
    : QWidget(parent), ui(std::make_unique<Ui::ResultWidget>()) {
    ui->setupUi(this);
...
ResultWidget::~ResultWidget() { clear(); }
...
void ResultWidget::setupConnections() {
    ...
    connect(ui->ChannelBox, &QCheckComboBox::itemStatusChanged, this,
            &ResultWidget::updateImages);
    connect(ui->ImageBox, &QCheckComboBox::itemStatusChanged, this,
            &ResultWidget::updateImages);

    // Connect layout control change signals
    connect(ui->rowSpin, &QSpinBox::valueChanged, ui->contentWidget,
            &QImagesWidget::setRowNum);
    connect(ui->columnSpin, &QSpinBox::valueChanged, ui->contentWidget,
            &QImagesWidget::setColNum);
    connect(ui->widthSpin, &QSpinBox::valueChanged, ui->contentWidget,
            &QImagesWidget::setViewWidth);
    connect(ui->heightSpin, &QSpinBox::valueChanged, ui->contentWidget,
            &QImagesWidget::setViewHeight);
}
...
void ResultWidget::updateImages() {
    ...
    QList<QVariant> checkedChannels =
        ui->ChannelBox->values(QCheckComboBox::CHECKED);
    ...
    // Prepare image list
    QList<QImage> images;
    images.reserve(checkedChannels.size() *
                   checkedImages.size()); // Preallocate memory
    ...
    for (const QVariant &channel : checkedChannels) {
        ...
        for (const QVariant &image : checkedImages) {
            ...
            if (imageIndex < 0 || imageIndex >= m_channels[channelIndex].size())
                continue;
